...
SESSION_STRING = "BQH6T14AAJTjZpMC1EmZQ8SQ4qRYEejNYxPUQgGYuqXllq-VZaWLQiuTedXk61w96bj_ke5IDmr2bmWny4CEwW2jdO0f3nizEFWRdNP_dr6bjlN02NQdLVvKAHQCHV98hmDmGiDApXt-oqN-E94GriwznXvaRsQp3ML9FvqF-4NUg9U9K_DWf9Yd5gy2GCV0Y2oMNk2WbB9ZK4Xa0N36bBwVq-Z6yWfSdkm2_zUf8U1LAMfXmOD-lNI0qYQRm9xfOmXZ9cl-wP5heB7cfWPJn0CvgqGdWtn_wS7bArH1Bv7PgjhRIs-wRZvulE36bC2ED7t0lC9A-jV-7F12K0rHMPeE6vujlQAAAAILtc1CAA"
OWNER_ID = 8242523973
...
async def _fetch_series_episodes(session, base_url, user, passwd, series_info):
    """Helper function to fetch episodes for a single series."""
...
async def add_xtream_series_optimized(client, message):
    ...
    try:
        ...
        await msg.edit_text(f"✅ **Scan Complete!**\nLoaded episodes for **{processed_count}** series from Xtream API!{CREDITS}", disable_web_page_preview=True)

    except Exception as e:
        await msg.edit_text(f"❌ An error occurred: `{str(e)[:100]}`")
...
# ================= Playback & Conflict Resolution (Unchanged but vital) =================
...
async def _start_stream(client, message, name, media_type):
    ...
    try:
        await call_py.play(chat_id, MediaStream(url, video_parameters=VideoQuality.HD_720p))
        ...
        await msg.edit_text(f"╭━━━[ **▶️ Now Streaming** ]━━━╮\n┃\n┃ ❖ **Title:** {display_name}\n┃ ❖ **Quality:** 720p (HD)\n┃\n╰━━━━━━━━━━━━━━━━━╯{CREDITS}", reply_markup=keyboard, disable_web_page_preview=True)
    except Exception as e:
        await msg.edit_text(f"❌ **Stream Failed!**\n`{str(e)[:100]}`{CREDITS}", disable_web_page_preview=True)


# ================= All Other Commands (No changes needed) =================
# ... [The existing code for /start, add/del commands, admin commands, stopvc, etc., remains unchanged]
...
async def start_cmd(client, message):
    await message.reply(f"╭━━━[ **{BOT_NAME}** ]━━━╮\n"
        ...
        f"┃\n"
        f"╰━━━━━━━━━━━━━━━━━╯{CREDITS}", disable_web_page_preview=True)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url, timeout=60) as resp:
                ...
                else: await msg.edit_text(f"❌ Error Status: {resp.status}")
    except Exception as e: await msg.edit_text(f"❌ Failed: `{str(e)[:100]}`")
@app.on_message(filters.command("delallchannels") & filters.user(OWNER_ID))
async def del_all_channels(client, message):
    CHANNELS.clear(); await message.reply(f"🗑️ **All Live TV channels deleted.**{CREDITS}", disable_web_page_preview=True)
@app.on_message(filters.command("delallmovies") & filters.user(OWNER_ID))
async def del_all_movies(client, message):
    MOVIES.clear(); await message.reply(f"🗑️ **All movies deleted.**{CREDITS}", disable_web_page_preview=True)
@app.on_message(filters.command("delallseries") & filters.user(OWNER_ID))
async def del_all_series(client, message):
    SERIES.clear(); await message.reply(f"🗑️ **All series deleted.**{CREDITS}", disable_web_page_preview=True)
@app.on_message(filters.command("addchannel") & filters.user(OWNER_ID))
async def add_manual_channel(client, message):
    args = message.text.split(None, 2)
    if len(args) < 3: return await message.reply(f"⚠️ **Usage:** `/addchannel <URL> <Channel Name>`{CREDITS}", disable_web_page_preview=True)
...
async def approve_group(client, message):
    ...
    except (ValueError, IndexError): await message.reply("Please specify a Chat ID or use in the group.")
...
async def broadcast(client, message):
    ...
    for chat_id in APPROVED_GROUPS:
        try:
            await app.send_message(chat_id, f"🔔 **Broadcast**\n\n{msg_text}{CREDITS}", disable_web_page_preview=True); success += 1
        except: failed += 1
...
async def switch_quality(client, callback_query):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(q.upper(), callback_data=f"q_{q}") for q in ["360p", "480p", "720p", "1080p"]]])
        await callback_query.message.edit_text(f"╭━━━[ **▶️ Now Streaming** ]━━━╮\n┃\n"
            f"┃ ❖ **Title:** {stream_info['name']}\n"
            f"┃ ❖ **Quality:** {quality_req.upper()} ✅\n"
            f"┃\n"
            f"╰━━━━━━━━━━━━━━━━━╯{CREDITS}", reply_markup=keyboard, disable_web_page_preview=True)
        await callback_query.answer(f"✅ Switched to {quality_req}!")
    except Exception as e: await callback_query.answer(f"❌ Failed to switch!", show_alert=True)
...
async def stop_vc(client, message):
    ...
    except: await message.reply(f"❌ **No active stream to stop.**{CREDITS}", disable_web_page_preview=True)
...
async def main():
    ...
    try: await app.send_message(OWNER_ID, f"🟢 **{BOT_NAME} Online!**{CREDITS}", disable_web_page_preview=True)
    except: pass
    await idle()
